...
const PRICE_TIER = {'经济':1,'主流':2,'高端':3,'豪华':4};
const TIER_SHORT = {1:'≤15万',2:'15–25万',3:'25–40万',4:'40万+'};
const TIER_MID   = {1:12,2:20,3:32,4:55};   // 各档中位价（万），用于估“向往 vs 现实”差额
const MIN_SWIPE = 6;
const SEED = 5;
let main;
...
function parseQuiz(answers){
  ...
  return {
    budgetTier: o.budget ? o.budget.tier : null,
...
function scoreCatalog(liked, disliked, quiz){
  ...
  return CATALOG.map(car=>{
    ...
    const tier = PRICE_TIER[car.price];
    let inBudget=true, seatFit=true;
    if(quiz.budgetTier){ if(tier<=quiz.budgetTier) s+=6; else { s-=(tier-quiz.budgetTier)*14; inBudget=false; } }
...
function gapInsight(liked, quiz){
  const out = [];
  // 预算差
  const tiers = liked.map(c=>PRICE_TIER[c.price]).filter(Boolean);
  const aspTier = tiers.length ? Math.round(tiers.reduce((a,b)=>a+b,0)/tiers.length) : null;
  if(aspTier && quiz.budgetTier && aspTier > quiz.budgetTier){
    const diff = TIER_MID[aspTier]-TIER_MID[quiz.budgetTier];
    out.push({ kind:'budget',
      title:'眼睛比钱包贵了一档',
      body:`你右滑的车均价落在 ${TIER_SHORT[aspTier]} 档，可你给的预算是 ${TIER_SHORT[quiz.budgetTier]}——中间差着约 ${diff} 万。下面的推荐我已按你的预算来，但你的“向往款”也一并标出来，心里有数。` });
  }
  // 玩乐 vs 家用
...
function cohortOf(liked, quiz){
  ...
  const chips = [
    quiz.first ? '首购' : '增换购',
    quiz.budgetTier ? TIER_SHORT[quiz.budgetTier] : '预算不限',
    `${domVibe}·${power}倾向`,
    quiz.intentLabel,
  ];
  // 价值分档（示意）：意图 × 首购
...
const SYS = '你是懂车又敢说真话的购车顾问。用户先在一堆车里左右滑（右滑=喜欢，采集隐式品味），再答了几道题（预算/座位/能源/时间线/是否首购，是显式约束与购买意图）。请据此精配，并把"向往 vs 现实"的张力说透。只输出严格 JSON：'+
  ...
  'picks 正好 3 台、id 来自候选库且不重复、必须在预算内、座位够；wildcard 同样来自候选库。全部简体中文。';
...
function reasonLocal(car, tend, quiz){
  ...
  const inB = quiz.budgetTier ? (PRICE_TIER[car.price]<=quiz.budgetTier) : true;
...
function wildReasonLocal(car, liked, quiz){
  ...
  const over = quiz.budgetTier && PRICE_TIER[car.price] > quiz.budgetTier;
...
async function getBrief(liked, disliked, quiz, useAI){
  ...
  if(useAI && liked.length){
    try{
      ...
      const qz = `预算上限：${quiz.budgetTier?TIER_SHORT[quiz.budgetTier]:'不限'}；座位需求：≥${quiz.seatNeed}（${quiz.useWord||'未填'}）；能源：${quiz.energyPref||'不限'}；时间线：${quiz.intentLabel}；${quiz.first?'人生首购':'增换购'}`;
      ...
      const picks = (Array.isArray(obj.picks)?obj.picks:[]).map(p=>{
        ...
        if(quiz.budgetTier && PRICE_TIER[car.price]>quiz.budgetTier) return null;     // 预算硬约束
...
async function showResult(liked, disliked, answers, fromLink){
  ...
  stage.appendChild(GG.el('div',{class:'card pad', style:{marginBottom:'16px', background:`linear-gradient(160deg,var(--accent-soft),#fff 62%)`}},
    ...
    GG.el('div',{class:'chips', style:{marginTop:'12px'}},
      ...
      GG.el('span',{class:'chip', style:{cursor:'default'}}, quiz.budgetTier?TIER_SHORT[quiz.budgetTier]:'预算不限'),
  ...
  const shareSpec={ slug:SLUG, title:'我的购车品味 DNA · '+persona.name,
    subtitle:(quiz.first?'首购 · ':'')+(quiz.budgetTier?TIER_SHORT[quiz.budgetTier]+' · ':'')+'先试驾：'+t1.car.name,
