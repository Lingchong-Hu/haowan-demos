...
function analyze(srcCanvas){
  ...
  return {
    ...
    warmNorm, lightNorm,
    undertone: isWarm?GG.T('暖调','Warm undertone'):GG.T('冷调','Cool undertone'), depth:isLight?GG.T('浅','Light'):GG.T('深','Deep'),
  };
}
...
function welcome(){
  ...
  const head = GG.el('div',{class:'sd-top'}, ...blobs,
    ...
    GG.el('div',{class:'sd-brand'}, GG.T('色彩 DNA','Color DNA')),
    GG.el('div',{class:'sd-tag'}, GG.T('AI 个人色彩诊断 · 30 秒读出你的颜色','AI personal color analysis · read your colors in 30 seconds')));
  ...
  const body = GG.el('div',{class:'sd-body'},
    GG.el('div',{class:'sd-hook', html:GG.T('一张自拍，<b>读懂你的颜色</b><br>该穿什么、该买什么、口红选几号','One selfie, <b>decode your colors</b><br>what to wear, what to buy, which lipstick shade')}),
    GG.el('div',{class:'sd-feats'},
      feat('🎯', GG.T('冷暖深浅 → 你的专属色彩季型 + 色彩人格','Undertone & depth → your personal color season + color persona')),
      feat('👗', GG.T('戴对色 vs 戴错色：一眼看出差别','Right color vs wrong color: see the difference at a glance')),
      feat('💄', GG.T('口红 / 腮红 / 金属 / 发色，直接抄色号','Lipstick / blush / metals / hair color — exact shades to copy'))),
    GG.el('div',{class:'sd-proof'},
      GG.el('div',{class:'sd-faces'}, ...proofFaces),
      GG.el('span', null, GG.T('已有 12,847 人找到了自己的颜色','12,847 people have found their colors'))),
    fileInput,
    GG.el('button',{class:'sd-go', onClick:()=>fileInput.click()}, GG.T('📷 上传自拍，开始诊断','📷 Upload a selfie to start')),
    GG.el('button',{class:'sd-wx', onClick:()=>{ GG.toast(GG.T('微信授权成功（演示）','WeChat sign-in successful (demo)')); useSample(SAMPLES[0]); }},
      GG.el('span',{class:'g'}, '❖'), GG.T('微信一键登录','Sign in with WeChat')),
    GG.el('div',{class:'sd-or'}, GG.T('没有合适的自拍？','No good selfie handy?')),
    GG.el('div',{class:'sd-samp-h'}, GG.T('挑一张示例脸，先看看完整效果','Pick a sample face to preview the full result')),
  ...
  main.appendChild(GG.el('div',{class:'sd-gate'},
    ...
    GG.el('div',{class:'sd-priv'},
      GG.el('span', null, '🔒'),
      GG.el('span', null, GG.T('照片只在你的浏览器本机处理，不上传任何服务器；演示环境不收集账号信息。','Photos are processed locally in your browser and never uploaded to any server; this demo collects no account info.')))));
}

function useSample(s){
  const c=document.createElement('canvas'); c.width=240; c.height=240;
  drawSample(c.getContext('2d'), s, 240, 240);
  run(c, c.toDataURL('image/png'), GG.T(s.label+'示例', s.label+' sample'));
}
function loadFile(f){
  ...
  reader.onload=()=>{ const img=new Image(); img.onload=()=>{
    ...
    run(c, c.toDataURL('image/png'), GG.T('你的自拍','Your selfie'));
...
async function run(canvas, dataURL, srcLabel){
  ...
  await GG.thinking(stage, [GG.T('读取照片像素…','Reading photo pixels…'),GG.T('量化主色…','Quantizing dominant colors…'),GG.T('判断冷暖与深浅…','Assessing undertone & depth…'),GG.T('匹配色彩人格…','Matching your color persona…'),GG.T('生成你的配色板…','Building your palette…')], 1600);
...
function colorMap(res){
  ...
  return GG.el('div',{class:'card pad', style:{marginBottom:'16px'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('你在色彩坐标里的位置','Where you sit on the color map')),
    GG.el('div',{class:'sd-map'},
      GG.el('span',{class:'q', style:{top:'8px', left:'10px'}}, GG.T('冷夏','Cool Summer')),
      GG.el('span',{class:'q', style:{top:'8px', right:'10px'}}, GG.T('暖春','Warm Spring')),
      GG.el('span',{class:'q', style:{bottom:'8px', left:'10px'}}, GG.T('冷冬','Cool Winter')),
      GG.el('span',{class:'q', style:{bottom:'8px', right:'10px'}}, GG.T('暖秋','Warm Autumn')),
      GG.el('span',{class:'ax', style:{top:'50%', left:'8px', transform:'translateY(-50%)'}}, GG.T('冷','Cool')),
      GG.el('span',{class:'ax', style:{top:'50%', right:'8px', transform:'translateY(-50%)'}}, GG.T('暖','Warm')),
      GG.el('span',{class:'ax', style:{top:'6px', left:'50%', transform:'translateX(-50%)'}}, GG.T('浅','Light')),
      GG.el('span',{class:'ax', style:{bottom:'6px', left:'50%', transform:'translateX(-50%)'}}, GG.T('深','Deep')),
      dot),
    GG.el('p',{class:'small muted', style:{margin:'10px 0 0'}},
      GG.T(`你落在 ${res.undertone}·${res.depth}色系 这一象限 —— ${S.name}。越靠角，特征越鲜明。`,
           `You land in the ${res.undertone} · ${res.depth} quadrant — ${S.name}. The closer to a corner, the more pronounced the traits.`)));
}
...
function compareBlock(res, dataURL){
  ...
  return GG.el('div',{class:'card pad', style:{marginBottom:'16px'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('戴对色 vs 戴错色','Right color vs wrong color')),
    GG.el('div',{class:'sd-compare'},
      frame(best, '✓ '+best.n, true),
      frame(worst, '✗ '+worst.n, false)),
    GG.el('p',{class:'small muted', style:{margin:'12px 0 0'}},
      GG.T(`同一张脸，左边「${best.n}」提气色、右边「${worst.n}」显疲惫 —— 这就是选对颜色的差别。`,
           `Same face: “${best.n}” on the left lifts your complexion, “${worst.n}” on the right washes you out — that is what wearing the right color does.`)));
}
...
function beautyBlock(res){
  ...
  return GG.el('div',{class:'card pad', style:{marginBottom:'16px'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('妆造速查 · 直接抄色号','Beauty cheat sheet · shades to copy')),
    GG.el('div',{class:'sd-beauty'},
      row(GG.T('口红','Lipstick'), [shadeChip(B.lip), B.lip2?shadeChip(B.lip2):null].filter(Boolean)),
      row(GG.T('腮红','Blush'), [shadeChip(B.blush)]),
      row(GG.T('金属配饰','Metals'), [GG.el('span',{class:'bt'}, B.metal)]),
      row(GG.T('发色','Hair color'), [shadeChip(B.hair)])),
    GG.el('p',{class:'small muted', style:{margin:'12px 0 0'}}, GG.T('色号是同色系参考，买前在自然光下贴脸比一比最准。','Shades are same-family references — hold them up to your face in natural light before buying.')));
}
...
function wardrobeBlock(res){
  ...
  return GG.el('div',{class:'card pad', style:{marginBottom:'16px'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('衣柜断舍离','Wardrobe edit')),
    GG.el('div',{class:'sd-ward'},
      col('keep',GG.T('✓ 该留 / 多买','✓ Keep / buy more'), B.keep),
      col('toss',GG.T('✗ 别再踩雷','✗ Stop buying'), B.toss)));
}

/* ---------- AI 造型顾问（连了 key 才叠加） ---------- */
const SD_SYS = '你是资深色彩造型顾问。用户做完色彩季型诊断，下面是结果。请给出更个性化、可执行的穿搭与妆容建议。只输出严格 JSON：{"summary":"一句话点评 ta 的色彩特质","wear":["3条具体的单品/颜色搭配建议"],"makeup":["2条妆容或配饰建议"],"avoid":"一句话提醒最该避开什么"}。'+GG.T('全部简体中文，建议要具体（点名颜色/单品），不要空话。','所有输出文本一律用英文（English only），建议要具体（点名颜色/单品），不要空话。');
...
function mountAdvice(stage, res){
  ...
  const head = GG.el('div',{class:'row', style:{justifyContent:'space-between', alignItems:'center'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('✨ AI 造型顾问','✨ AI Styling Advisor')),
  ...
  if(!GG.llm.connected()){
    body.appendChild(GG.el('p',{class:'small muted', style:{margin:'0 0 10px'}},
      GG.T('上面的季型 / 配色 / 色号都是本机即时算出来的。连上 AI 后，这里会多一份「为你这张脸量身写」的穿搭与妆容建议。','Everything above — season, palette, shades — is computed locally in real time. Connect AI to add outfit and makeup advice written specifically for your face.')));
    body.appendChild(GG.llm.bar(()=>{ start(); /* 连接状态变化后让用户重测以看到 AI 段落 */ }));
    return;
  }
  body.appendChild(GG.el('p',{class:'small muted', style:{margin:'0'}}, GG.T('正在为你的季型生成个性化建议…','Generating personalized advice for your season…')));
  aiAdvice(res).then(obj=>{
    ...
    if(wear.length){ body.appendChild(GG.el('div',{class:'section-t', style:{marginTop:'4px'}}, GG.T('穿搭','Outfits'))); body.appendChild(bullets(wear)); }
    if(makeup.length){ body.appendChild(GG.el('div',{class:'section-t'}, GG.T('妆容 / 配饰','Makeup / Accessories'))); body.appendChild(bullets(makeup)); }
    if(obj.avoid) body.appendChild(GG.el('p',{class:'small muted', style:{margin:'10px 0 0'}}, GG.T('避开：','Avoid: ')+String(obj.avoid)));
    if(!wear.length && !makeup.length && !obj.summary) body.appendChild(GG.el('p',{class:'small muted', style:{margin:'0'}}, GG.T('这次没生成出建议，核心诊断不受影响。','No advice came through this time — your core analysis is unaffected.')));
  }).catch(e=>{ GG.clear(body);
    body.appendChild(GG.el('p',{class:'small muted', style:{margin:'0'}},
      GG.T('AI 建议没拿到（','AI advice unavailable (')+(e&&e.code||'NET')+GG.T('），核心色彩诊断不受影响。','), your core color analysis is unaffected.'))); });
}

function renderResult(stage, res, dataURL, srcLabel){
  const S=res.season, B=res.beauty;
  stage.appendChild(GG.el('div',{class:'hero', style:{paddingTop:'8px'}},
    GG.el('h1',{style:{fontSize:'24px'}}, GG.T('🎨 你的色彩 DNA','🎨 Your Color DNA'))));
  ...
  stage.appendChild(GG.el('div',{class:'card pad', style:{display:'flex', gap:'16px', alignItems:'center', flexWrap:'wrap', marginBottom:'16px',
    ...
    GG.el('div',{style:{flex:'1', minWidth:'200px'}},
      GG.el('div',{class:'section-t', style:{marginTop:'0', marginBottom:'4px'}}, GG.T('色彩季型 · 色彩人格','Color Season · Color Persona')),
      GG.el('div',{style:{fontSize:'28px', fontWeight:'760', lineHeight:'1.15'}}, S.name),
      GG.el('div',{style:{fontSize:'15px', fontWeight:'650', color:'var(--accent)', marginTop:'2px'}}, GG.T('「'+B.persona+'」','“'+B.persona+'”')),
      GG.el('div',{class:'kpi', style:{marginTop:'9px'}},
        GG.el('span',{class:'pill'}, res.undertone),
        GG.el('span',{class:'pill'}, GG.T(res.depth+'色系', res.depth)),
  ...
  stage.appendChild(GG.el('div',{class:'card pad', style:{marginBottom:'16px'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('从你的照片里取到的主色','Dominant colors pulled from your photo')),
    swRow(res.dominant, true),
    GG.el('p',{class:'small muted', style:{margin:'12px 0 0'}},
      GG.T(`共量化 ${res.dominant.length} 个主色：${res.dominant.map(d=>d.name).join('、')}。冷暖依据全图红蓝差判定。`,
           `Quantized ${res.dominant.length} dominant colors: ${res.dominant.map(d=>d.name).join(', ')}. Undertone is judged from the photo's overall red–blue difference.`))
  ));

  // 推荐配色板 + 避开
  stage.appendChild(GG.el('div',{class:'card pad', style:{marginBottom:'16px'}},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('为你生成的专属配色板','Your personal palette')),
    swRow(res.palette, true),
    GG.el('p',{class:'small', style:{margin:'12px 0 6px', color:'var(--ink-2)'}}, S.advice),
    GG.el('div',{class:'section-t'}, GG.T('建议避开','Colors to avoid')),
  ...
  const shareSpec={
    slug:SLUG, title:GG.T('我的色彩季型 · '+S.name+'「'+B.persona+'」','My Color Season · '+S.name+' “'+B.persona+'”'),
    subtitle:GG.T(`${res.undertone} / ${res.depth}色系 · ${S.vibe}`, `${res.undertone} / ${res.depth} · ${S.vibe}`),
    swatches: res.palette,
    note: B.hook,
    rows:[ {label:GG.T('口红色号','Lipstick shades'), value:B.lip.n+(B.lip2?(' / '+B.lip2.n):'')},
           {label:GG.T('金属配饰','Metals'), value:B.metal} ],
    tags: res.dominant.slice(0,4).map(d=>d.name),
  };
  stage.appendChild(GG.resultCard(SLUG, GG.el('div',{class:'center muted small'}, GG.T('存图分享你的色彩 DNA ↓','Save & share your Color DNA ↓')), shareSpec));
  stage.appendChild(GG.el('div',{class:'center', style:{marginTop:'18px'}},
    GG.el('button',{class:'btn', onClick:()=>{ start(); }}, GG.T('↻ 换张照片','↻ Try another photo'))));
}
