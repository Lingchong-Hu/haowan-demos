...
function reasonSentence(qid, userKey, name){
  ...
  switch(qid){
    case 'weekend':
      return GG.T(`你的理想周末是「${yours}」，${name} 的周末也正是这样过的。`,
                  `Your ideal weekend is “${yours}” — and that’s exactly how ${name} spends theirs.`);
    case 'comm':
      return GG.T(`你看重的沟通是「${yours}」，${name} 恰好就是这种相处方式。`,
                  `The communication you value is “${yours}” — that happens to be exactly how ${name} connects.`);
    case 'value':
      return GG.T(`你最希望对方「${yours}」，而这正是 ${name} 身上最被人记住的一点。`,
                  `You most hope your person is “${yours}” — and that’s the very thing people remember about ${name}.`);
    case 'deal':
      return GG.T(`你最受不了的是「${yours}」，${name} 也把这条列为自己的雷点——你俩的底线对上了。`,
                  `What you can’t stand is “${yours}” — ${name} lists it as a dealbreaker too. Your bottom lines match.`);
    case 'pace':
      return GG.T(`关系节奏上你想要「${yours}」，${name} 想要的节奏和你一致。`,
                  `For pace, you want “${yours}” — ${name} wants the exact same rhythm.`);
    case 'self':
      return GG.T(`你给自己贴的标签是「${yours}」，${name} 也是同一挂的。`,
                  `The label you gave yourself is “${yours}” — ${name} is cut from the same cloth.`);
    default:
      return GG.T(`你们在「${dimOf(qid)}」上选了同一个答案：「${yours}」。`,
                  `You both picked the same answer on “${dimOf(qid)}”: “${yours}”.`);
  }
}
...
function frictionSentence(qid, userKey, taKey, name){
  ...
  switch(qid){
    case 'weekend':
      return GG.T(`周末你想「${yours}」，${name} 更想「${theirs}」。不必强求一致——轮流安排，反而能把对方的世界也逛一遍。`,
                  `For weekends you’d rather “${yours}”, while ${name} leans toward “${theirs}”. No need to match — take turns planning, and you each get a tour of the other’s world.`);
    case 'comm':
      return GG.T(`沟通上你要的是「${yours}」，${name} 习惯「${theirs}」。一开始可能会错频，但说开之后，这两种方式其实挺互补。`,
                  `In communication you want “${yours}”, while ${name} is used to “${theirs}”. You may misread each other at first, but once it’s out in the open, the two styles actually complement each other.`);
    case 'value':
      return GG.T(`你最看重「${yours}」，${name} 身上更突出的是「${theirs}」。看重的点不一样，恰好能在对方身上补上自己缺的那块。`,
                  `You value “${yours}” most, while what stands out in ${name} is “${theirs}”. Caring about different things means you each fill in the piece the other is missing.`);
    case 'pace':
      return GG.T(`节奏上你想「${yours}」，${name} 想「${theirs}」。这点最值得第一次见面就轻轻聊一句——各让一步，别让"快慢差"攒成误会。`,
                  `For pace, you want “${yours}” and ${name} wants “${theirs}”. Worth a gentle mention on the very first date — meet halfway, so the speed gap never piles up into a misunderstanding.`);
    case 'self':
      return GG.T(`你给自己贴「${yours}」，${name} 是「${theirs}」。两种活法凑到一起，新鲜感往往比"同类"更耐放。`,
                  `You call yourself “${yours}”, while ${name} is “${theirs}”. When two ways of living meet, the freshness often lasts longer than sameness.`);
    default:
      return GG.T(`你们在「${dimOf(qid)}」上选得不一样：你「${yours}」、${name}「${theirs}」——差异有时反而是聊不完的开头。`,
                  `You chose differently on “${dimOf(qid)}”: you “${yours}”, ${name} “${theirs}” — sometimes a difference is exactly what keeps the conversation going.`);
  }
}
...
function match(answers){
  ...
  const scored = CANDIDATES.map((c, idx)=>{
    ...
    // 用语言无关的 c.key（中文模式下与原 c.name 相同）——中英文下匹配结果一致
    const jitter = GG.rng(seed+'#'+(c.key||c.name))()*0.4;
  ...
  if(reasons.length < 3){
    ...
    for(const q of QUESTIONS){
      ...
      reasons.push({ qid:q.id, text:
        GG.T(`你在「${q.dim}」选了「${labelOf(q.id, ak)}」，${best.c.name} 的取向略有不同，但 TA 说这种差异反而聊得起来。`,
             `On “${q.dim}” you picked “${labelOf(q.id, ak)}” — ${best.c.name} leans a little differently, but says that kind of difference is what keeps a conversation alive.`) });
    }
  }
...
const MM_SYS = '你是温暖、走心、说真话的红娘。系统已从候选库里为用户选定了唯一一位 TA（不可更换）。下面给你 TA 的资料、用户的问卷答案、你俩完全对上的维度、以及你俩最该磨合的那个差异维度。请写出：①你们为什么对得上的理由 ②一句可直接发出去的破冰开场白 ③一句"红娘悄悄话"，点出你俩最该磨合的那一点。只输出严格 JSON：{"reasons":["3到4条理由，每条引用用户的具体答案+TA的特点，温暖具体，35字内"],"icebreaker":"一句自然、不油腻、可直接发出去的破冰开场白","friction":"一句话点出你俩最该磨合的那点，引用下面给的差异维度与双方具体选择，温和成熟、并给一句怎么相处的建议，40字内，绝不说成缺点或劝退"}。绝对不要出现任何匹配度、分数、百分比或数字评分。'+GG.T('全部简体中文。','所有输出字段（reasons/icebreaker/friction）一律用温暖自然的英文（English）书写。');
...
function intro(){
  ...
  main.appendChild(GG.el('div',{class:'hero'},
    GG.el('h1', null, GG.T('回答几道题，我只给你一个人','Answer a few questions — I’ll give you just one person')),
    GG.el('p', null, GG.T(`不是滑不完的列表，也不算什么"匹配度"。${QUESTIONS.length} 道关于你的小问题，答完我会从候选库里，挑出最该认识你的那一位——告诉你为什么是 TA，连你俩唯一要磨合的那一点，也照实说。`,
      `No endless list to swipe, no “compatibility score”. Just ${QUESTIONS.length} little questions about you — then I’ll pick, from the whole pool, the one person who most deserves to meet you. I’ll tell you why it’s them, and honestly name the one thing you two will need to work through.`))
  ));
  main.appendChild(GG.llm.bar());
  main.appendChild(GG.el('div',{class:'center', style:{marginTop:'22px'}},
    GG.el('button',{class:'btn primary lg', onClick:()=>quiz()}, GG.T('开始回答 →','Start answering →'))
  ));
}
...
function quiz(){
  ...
  function render(){
    const q = QUESTIONS[i];
    GG.clear(head);
    head.appendChild(GG.el('div',{class:'small muted'}, GG.T(`第 ${i+1} / ${QUESTIONS.length} 题　·　${q.dim}`, `Question ${i+1} / ${QUESTIONS.length} · ${q.dim}`)));
    ...
    GG.clear(nav);
    nav.appendChild(i>0
      ? GG.el('button',{class:'btn ghost', onClick:back}, GG.T('← 上一题','← Previous'))
      : GG.el('span'));
    nav.appendChild(GG.el('div',{class:'small muted'}, GG.T('选一个就自动进入下一题','Pick one to move on automatically')));
  }
...
async function showResult(answers, fromLink){
  ...
  if(!fromLink){
    const think = GG.thinking(stage, [
      GG.T('读你刚才的每一个选择…','Reading every choice you just made…'),
      GG.T('比对候选库里每个人的答案…','Comparing answers from everyone in the pool…'),
      useAI ? GG.T('让 AI 把你们对上的点说清楚…','Letting AI spell out what clicked between you…')
            : GG.T('找你们真正对得上的那些点…','Finding the points where you truly align…'),
      GG.T('只留一个最该认识你的人…','Keeping just the one person who should meet you…'),
    ], useAI?2000:1700);
  ...
  stage.appendChild(GG.el('div',{class:'hero', style:{paddingTop:'8px'}},
    GG.el('h1',{style:{fontSize:'24px'}}, GG.T('💞 为你精配的，就这一位','💞 Matched just for you — this one person'))));
  ...
  const why = GG.el('div',{class:'card pad'},
    GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('你们对上的点','Where you two click')),
  ...
  if(friction){
    stage.appendChild(GG.el('div',{class:'card pad', style:{marginTop:'16px', borderLeft:'3px solid var(--warn)', background:'#fffaf2'}},
      GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('🤫 红娘悄悄话：你俩要磨合的一点','🤫 Matchmaker’s whisper: the one thing to work through')),
      GG.el('p',{style:{margin:'0 0 8px', fontSize:'15px', color:'var(--ink)', lineHeight:'1.7'}}, friction),
      GG.el('p',{class:'small muted', style:{margin:'0'}}, GG.T('别的 App 只会让你看着般配——这一条，才是真红娘会提前说的话。','Other apps only show you how well-matched you look — this line is what a real matchmaker tells you upfront.'))
    ));
  }
  ...
  if(icebreaker){
    stage.appendChild(GG.el('div',{class:'card pad', style:{marginTop:'16px', borderLeft:'3px solid var(--accent)'}},
      GG.el('div',{class:'section-t', style:{marginTop:'0'}}, GG.T('✨ 不知道怎么开口？试试这句','✨ Not sure how to start? Try this line')),
  ...
  const shareSpec = {
    slug: SLUG,
    title: GG.T('为你精配：','Matched for you: ')+cand.name,
    subtitle: GG.T(cand.age+' 岁 · '+cand.city, cand.age+' · '+cand.city),
    rows: reasons.map((r,idx)=> ({ label:GG.T('对上的点 '+(idx+1), 'In sync #'+(idx+1)), value: r.text })),
    tags: cand.tags,
    note: GG.T(cand.name+'——不是匹配度最高，是你的答案里，TA 一条条都接得住。',
               cand.name+' — no “top match score” here; it’s that your answers, one by one, all land with them.'),
  };
  // 悄悄话也进分享卡：般配 + 要磨合的，一起截出去才是这版的记忆点
  if(frictionMeta) shareSpec.rows.push({ label:GG.T('🤫 要磨合的','🤫 To work through'),
    value: GG.T(frictionMeta.dim+'：你「'+frictionMeta.yours+'」· TA「'+frictionMeta.theirs+'」',
                frictionMeta.dim+': you “'+frictionMeta.yours+'” · them “'+frictionMeta.theirs+'”') });

  stage.appendChild(GG.resultCard(SLUG,
    GG.el('div',{class:'center muted small'}, GG.T('把 TA 截图分享出去 ↓','Screenshot your match and share it ↓')),
    shareSpec));
  ...
  stage.appendChild(GG.el('div',{class:'center', style:{marginTop:'18px'}},
    GG.el('button',{class:'btn', onClick:()=>{ location.hash=''; main = GG.mountShell(SLUG); intro(); }},
      GG.T('↻ 换一组答案重新匹配','↻ Answer again, match again'))
  ));
}
